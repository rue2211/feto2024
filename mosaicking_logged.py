...
Addition of evaluating the results with local SSIM (Structural Similartity Index)
"""
...
def calculate_local_ssim(mosaic_image, original_images, transformations):
    """Calculate SSIM for each original image against its corresponding region in the mosaic."""
    mosaic_image_float = img_as_float(mosaic_image)
    ssim_scores = []

    for i, (orig_image, transform) in enumerate(zip(original_images, transformations)):
        # Warp the original image to the mosaic's coordinate space
        warped_image = cv2.warpPerspective(orig_image, transform, (mosaic_image.shape[1], mosaic_image.shape[0]))

        # Convert images to grayscale if necessary
        if len(mosaic_image_float.shape) == 3 and mosaic_image_float.shape[2] == 3:
            mosaic_image_gray = cv2.cvtColor((mosaic_image_float * 255).astype(np.uint8), cv2.COLOR_BGR2GRAY)
            warped_image_gray = cv2.cvtColor((img_as_float(warped_image) * 255).astype(np.uint8), cv2.COLOR_BGR2GRAY)
        else:
            mosaic_image_gray = (mosaic_image_float * 255).astype(np.uint8)
            warped_image_gray = (img_as_float(warped_image) * 255).astype(np.uint8)

        # Calculate SSIM for the overlapping region only
        overlap_mask = (warped_image_gray > 0).astype(np.uint8)
        if np.sum(overlap_mask) > 0:
            ssim_index, _ = ssim(mosaic_image_gray, warped_image_gray, full=True)
            ssim_scores.append(ssim_index)

    # Calculate average SSIM score
    avg_ssim = np.mean(ssim_scores) if ssim_scores else 0
    print(f"Average SSIM Index: {avg_ssim}")
...
registered_images, transformations, frame_transformations = register_images_affine(conf_maps, orig_images, csv_filename="100/image_transformationsSSIM.csv")
...
plt.savefig('100/image_mosaicSSIM.png')
plt.show()

# Calculate and print the average SSIM score
avg_ssim = calculate_local_ssim(mosaic, orig_images, transformations)
print(f"The average SSIM index for the mosaic is: {avg_ssim}")
